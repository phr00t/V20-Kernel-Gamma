...
IS_ENABLED(CONFIG_LGE_DISPLAY_OVERRIDE_MDSS_DSI_PANEL_POWER_OFF)
static int lge_dsi_panel_power_onoff_precheck(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (ctrl_pdata->panel_data.panel_info.aod_labibb_ctrl != true){
		pr_err("[Display] %s aod_labibb_ctrl is false\n", __func__);
		return 1;
	}
#endif
	return 0;
...
int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	ret = lge_dsi_panel_power_onoff_precheck(ctrl_pdata);
	if(ret)
		return 0;
